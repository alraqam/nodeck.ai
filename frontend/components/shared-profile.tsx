...
/**
 * The shared profile, rendered on the server.
 *
 * Deliberately not a client component. A share link's whole life is being
 * pasted into an email or a chat, and every unfurler, crawler and reader mode
 * that follows it runs no JavaScript - a client-rendered page hands all of
 * them an empty shell. Only the theme toggle is interactive, and it is a
 * client island inside an otherwise static page.
 */
...
/** Only http(s) links are rendered as links. A profile is user-supplied, and a
 *  `javascript:` or `data:` href on a page strangers open is not acceptable. */
...
export function SharedProfileUnavailable() {
    return (
        <SharedProfileShell>
...
export function SharedProfile({ profile }: { profile: PublicProfile }) {
    const { score } = profile
    const website = safeUrl(profile.identity.website)
            ...
            <article className="space-y-8 py-12">
                ...
                <div className="grid gap-3 lg:grid-cols-2">
                    <Section title="Problem">
                        ...
                        {profile.problem.validated && (
                            <Badge variant="high">Customer validated</Badge>
                        )}
                    </Section>
